...
exports.optimizedCrawlerService = exports.OptimizedCrawlerService = exports.OptimizedUnOchaCrawler = exports.OptimizedAlJazeeraEthiopiaCrawler = exports.OptimizedBbcEthiopiaCrawler = exports.OptimizedTestCrawler = exports.OptimizedBaseCrawler = exports.CrawlerError = void 0;
const puppeteer = __importStar(require("puppeteer"));
// Enhanced logging utilities
class CrawlerLogger {
    static formatMessage(level, source, message, extra) {
...
class OptimizedBaseCrawler {
    ...
    async crawl() {
        ...
        try {
            ...
            const crawlerError = error instanceof CrawlerError ? error :
                new CrawlerError(error instanceof Error ? error.message : 'Unknown crawling error', this.config.name, 'UNKNOWN', this.config.baseUrl, error instanceof Error ? error : undefined);
    ...
    async performCrawl() {
        ...
        try {
            ...
            const links = await this.page.$$eval(this.config.selectors.articleLinks, (anchors, baseUrl) => {
                return anchors
                    .map(anchor => anchor.href)
                    .filter(href => href && href.startsWith('http'))
                    .slice(0, 10); // Get more links initially for filtering
            }, this.config.baseUrl).catch(() => []);
            if (links.length === 0) {
    ...
    async extractArticle(url) {
        ...
        try {
            ...
            const title = await this.page.$eval(this.config.selectors.title, element => element.textContent?.trim() || '').catch(async () => {
                // Fallback selectors
     ...
     */
    generateArticleId(title, source, url) {
        const crypto = require('crypto');
        const id = crypto.createHash('md5').update(url).digest('hex');
        return id;
    }
}
exports.OptimizedBaseCrawler = OptimizedBaseCrawler;
/**
 * Test crawler for proof-of-concept and testing
 */
class OptimizedTestCrawler extends OptimizedBaseCrawler {
    constructor() {
        super({
            name: 'Test Crawler',
...
exports.OptimizedTestCrawler = OptimizedTestCrawler;
/**
 * BBC Ethiopia News Crawler - Enhanced with multiple fallback strategies
 */
class OptimizedBbcEthiopiaCrawler extends OptimizedBaseCrawler {
    constructor() {
        super({
    ...
    async crawlBbcAfricaPage() {
        ...
        const links = await this.page.$$eval('a[href*="/news/world-africa-"], a[href*="/news/"][data-testid*="internal-link"]', (anchors) => {
            return anchors
                .map(anchor => {
    ...
    async crawlBbcTopicPage() {
        ...
        for (const selector of selectors) {
            try {
                const foundLinks = await this.page.$$eval(selector, (anchors) => anchors
                    .map(anchor => anchor.href)
    ...
    async searchBbcForEthiopia() {
        ...
        try {
            ...
            const links = await this.page.$$eval('a[href*="/news/"]', (anchors) => anchors
                .map(anchor => anchor.href)
                .filter(href => href && href.startsWith('http') && href.includes('/news/'))
                .slice(0, 5)).catch(() => []);
            return await this.processArticleLinks(links);
    ...
    async processArticleLinks(links) {
        const articles = [];
        const maxArticles = this.config.maxArticles || 3;
    ...
    isValidEthiopiaArticle(article) {
        const title = article.title.toLowerCase();
        const snippet = article.snippet.toLowerCase();
        // Must be substantial
        if (title.length < 10 || snippet.length < 50)
            return false;
        ...
        return ethiopiaKeywords.some(keyword => title.includes(keyword) || snippet.includes(keyword));
    }
    getCuratedBbcContent() {
...
exports.OptimizedBbcEthiopiaCrawler = OptimizedBbcEthiopiaCrawler;
/**
 * Al Jazeera Ethiopia News Crawler - Focuses on Ethiopia-specific content
 */
class OptimizedAlJazeeraEthiopiaCrawler extends OptimizedBaseCrawler {
    constructor() {
        super({
    ...
    async performCrawl() {
        ...
        try {
            ...
            const links = await this.page.$$eval(this.config.selectors.articleLinks, (anchors) => {
                return anchors
                    .map(anchor => anchor.href)
                    .filter(href => {
    ...
    isValidArticle(article) {
        ...
        const mentionsEthiopia = ethiopiaKeywords.some(keyword => title.includes(keyword) || snippet.includes(keyword));
        if (!mentionsEthiopia) {
...
exports.OptimizedAlJazeeraEthiopiaCrawler = OptimizedAlJazeeraEthiopiaCrawler;
/**
 ...
 */
class OptimizedUnOchaCrawler extends OptimizedBaseCrawler {
    constructor() {
        super({
    ...
    async performSeleniumCrawl() {
        // Configure Chrome options for Selenium
        const chromeOptions = new chrome_js_1.Options();
        chromeOptions.addArguments('--headless', '--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu', '--window-size=1920,1080', '--disable-web-security', '--disable-features=VizDisplayCompositor', '--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
        ...
        const service = new chrome_js_1.ServiceBuilder(chromedriverPath);
        ...
        try {
            ...
            for (const selector of linkSelectors) {
                try {
                    links = await driver.findElements(selenium_webdriver_1.By.css(selector));
                    if (links.length > 0) {
            ...
            const articleLinks = await Promise.all(links.slice(0, 10).map(async (link) => {
                try {
            ...
            for (const articleLink of validLinks) {
                try {
                    ...
                    for (const selector of titleSelectors) {
                        try {
                            const titleElement = await driver.findElement(selenium_webdriver_1.By.css(selector));
                    ...
                    for (const selector of contentSelectors) {
                        try {
                            const contentElements = await driver.findElements(selenium_webdriver_1.By.css(selector));
                            if (contentElements.length > 0) {
                                const texts = await Promise.all(contentElements.slice(0, 3).map(async (elem) => {
                                    try {
...
exports.OptimizedUnOchaCrawler = OptimizedUnOchaCrawler;
/**
 * Optimized main crawler service
 */
class OptimizedCrawlerService {
    constructor() {
        this.crawlers = [];
    ...
    async crawlAll() {
        ...
        const startTime = Date.now();
        const results = await Promise.allSettled(this.crawlers.map(crawler => crawler.crawl()));
        const crawlerResults = results.map((result, index) => {
            if (result.status === 'fulfilled') {
                return result.value;
            }
            else {
                const crawlerName = this.crawlers[index] ? this.crawlers[index].config?.name : 'Unknown';
                return {
                    source: crawlerName,
                    articles: [],
...
exports.OptimizedCrawlerService = OptimizedCrawlerService;
// Export optimized singleton instance
exports.optimizedCrawlerService = new OptimizedCrawlerService();
